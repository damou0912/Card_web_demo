...
支持本地 1v1、标准 PVE、PVE 挑战、联网对战和 3x3/4x4/5x5 地图。PVE 挑战由精英 AI 执行：AI 开局多 1 张手牌，每回合额外获得 1 次行动，并在进入对局时随机抽取 1 个隐藏于主菜单、在对局内公布的精英特性。回合按准备、抽牌、回合开始技能、行动、结束清理、回合结束技能执行。每个行动阶段限时 120 秒，手牌上方显示倒计时，归零后自动结束回合；联网时由房主统一结算超时。开局生成两张中立守军，破坏格最多 5 个，牌库耗尽不生成援兵。左侧玩家信息固定为上方对方、下方我方，不随回合归属交换；联网时以本机玩家 ID 确定我方，行动数与计时器在对方回合切换为对方色。
