...
在执行行动前，将当前对局复制到隔离状态，在副本中执行与正式行动相同的技能事件链，再根据结算后的棋盘重新计算局面优势。模拟不会触发界面动画，也不会修改真实对局；带有随机目标的技能使用由行动内容派生的确定性随机序列评估，保证同一候选行动在比较时结果稳定。

AI 现在会对每个放置和移动候选执行该模拟，并将技能结算后的优势变化纳入排序；因此抽牌、强化、减益、自动攻击、保护、摧毁连锁、控制格和额外行动等效果都会影响最终选择。
