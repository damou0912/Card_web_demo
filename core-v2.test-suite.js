(function() {
  ...
  function coreRunV2CardBoundaryTests() {
    ...
    try {
      ...
      { const a = makeCard("03315", 1, 1, 1), enemy = makeCard("02101", 2, 1, 2), g = makeGame([a, enemy]); place(g, a); const placementDebuff = enemy.currentAttack === Math.max(0, enemy.attack - 2); const remote = makeCard("02102", 2, 3, 3); remote.currentAttack = 0; g.boardCards.push(remote); destroy(g, a); check("03315", "放置按两个数量条件减益，摧毁时销毁低于自身的全场敌军", placementDebuff && !g.boardCards.includes(enemy) && !g.boardCards.includes(remote)); }
